...
def main():
    ...
    if use_akochan:
        ...
        dqn = DQN().eval()
        state = torch.load(
            cfg["champion"]["state_file"], map_location=torch.device("cpu")
        )
        ...
        engine_cham = MortalEngine(
            ...
            enable_rule_based_agari_guard=cfg["champion"][
    ...
    if use_akochan and not all_akochan:
        ...
        dqn = DQN().eval()
        state = torch.load(
            cfg["challenger"]["state_file"], map_location=torch.device("cpu")
        )
        mortal.load_state_dict(state["mortal"])
        dqn.load_state_dict(state["current_dqn"])
        engine_chal = MortalEngine(
            ...
            enable_rule_based_agari_guard=cfg["challenger"][
                "enable_rule_based_agari_guard"
            ],
