...
def train():
    ...
    test_against = config["test_play"]["against"]
        ...
        for obs, _, actions, masks, steps_to_done, kyoku_rewards in data_loader:
            ...
            if steps % save_every == 0:
                ...
                if steps % test_every == 0:
                    ...
                    for name in test_against:
                        logging.info(f"test against {name}")
                        test_player = TestPlayer(name)
                        ...
                        writer.add_text(
                            f"test_play_{name}/report", str(stat).replace('\n','\n\n'), global_step=steps
                        )
                        writer.add_scalars(
                            "test_play_all/avg_ranking", {name: stat.avg_rank}, steps
                        )
                        writer.add_scalars(
                            "test_play_all/avg_pt", {name: avg_pt}, steps
                        )
                        writer.add_scalar(
                            f"test_play_{name}/avg_ranking", stat.avg_rank, steps
                        )
                        writer.add_scalar(f"test_play_{name}/avg_pt", avg_pt, steps)
                        writer.add_scalars(
                            f"test_play_{name}/ranking",
                            {
                        ...
                        writer.add_scalars(
                            "test_play_all/houjuu_point",
                            {name: stat.avg_point_per_houjuu},
                            steps,
                        )
                        writer.add_scalars(
                            "test_play_all/point_per_round",
                            {name: stat.avg_point_per_round},
                            steps,
                        )
                        writer.add_scalar(
                            f"test_play_{name}/houjuu_point",
                            stat.avg_point_per_houjuu,
                            steps,
                        )
                        writer.add_scalar(
                            f"test_play_{name}/point_per_round",
                            stat.avg_point_per_round,
                            steps,
                        )
                        writer.add_scalars(
                            f"test_play_{name}/key_step",
                            {
                        ...
                        writer.add_scalars(
                            "test_play_all/riichi_point",
                            {name: stat.avg_riichi_point},
                            steps,
                        )
                        writer.add_scalar(
                            f"test_play_{name}/riichi_point",
                            stat.avg_riichi_point,
                            steps,
                        )
                        writer.add_scalars(
                            f"test_play_{name}/fuuro",
                            {
                        ...
                        writer.add_scalars(
                            "test_play_all/fuuro_num", {name: stat.avg_fuuro_num}, steps
                        )
                        writer.add_scalars(
                            "test_play_all/fuuro_point",
                            {name: stat.avg_fuuro_point},
                            steps,
                        )
                        writer.add_scalar(
                            f"test_play_{name}/fuuro_num", stat.avg_fuuro_num, steps
                        )
                        writer.add_scalar(
                            f"test_play_{name}/fuuro_point", stat.avg_fuuro_point, steps
                        )
                        writer.add_scalars(
                            "time/test_time",
                            {
